...
deactivate()
{
    ...
    if (deactivate_return != true) {
        throw std::runtime_error(
                "Spectrometer failed to deactivate, device id incorrect"); 
    }
}
...
measure()
{
    ...
    unsigned int required_delay = 100; // ms
    int attempts = 0;
    const int MAX_ATTEMPTS = 100;
    int pollscan_return = AVS_PollScan(device_id);
	while(!pollscan_return && attempts != MAX_ATTEMPTS)
	{
        if (pollscan_return == SUCCESS) {
	...
	{
        int get_data_return = AVS_GetScopeData(
                device_id, time_label, data_buffer);
        if (get_data_return == SUCCESS || get_data_return == NO_DATA_IN_RAM) {
